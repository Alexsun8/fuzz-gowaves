...
func mustLoadEmbeddedSettings(blockchain BlockchainType) *BlockchainSettings {
	switch blockchain {
	case MainNet:
		s, err := loadEmbeddedSettings(mainnetSettingsJSON)
		if err != nil {
			panic(err)
		}
		return s
	...
	case TestNet:
		s, err := loadEmbeddedSettings(testnetSettingsJSON)
		if err != nil {
			panic(err)
		}
		return s
	...
	case StageNet:
		s, err := loadEmbeddedSettings(stagenetSettingsJSON)
		if err != nil {
			panic(err)
		}
		return s
...
func loadEmbeddedSettings(pubNetJSON string) (*BlockchainSettings, error) {
	return ReadBlockchainSettings(strings.NewReader(pubNetJSON))
}
